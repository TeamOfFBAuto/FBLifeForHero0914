...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        if (!_title_label) {
            _title_label = [[UILabel alloc] initWithFrame:CGRectMake(41,7,DEVICE_WIDTH-100,40)];
        ...
        if (!_follow_num_label) {
            _follow_num_label = [[UILabel alloc] initWithFrame:CGRectMake(DEVICE_WIDTH-130,0,60,54)];
        ...
        if (!_line_view) {
            _line_view = [[UIView alloc] initWithFrame:CGRectMake(DEVICE_WIDTH-61,12,0.5,30)];
        ...
        if (!_collection_button)
        {
            ...
            _collection_button.frame = CGRectMake(DEVICE_WIDTH-60,0,60,54);
...
-(void)setInfoWith:(int)ranking WithModel:(RankingListModel *)model WithType:(int)theType
{
    ...
    if (model.ranking_num.length != 0)
    {
        ...
        rect.size.width = DEVICE_WIDTH-180;
    }else
    {
        rect.size.width = DEVICE_WIDTH-120;
    }
