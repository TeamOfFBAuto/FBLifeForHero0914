...
@synthesize title_string = _title_string;
...
- (void)viewDidLoad
{
    ...
    _actionButton = [[UIBarButtonItem alloc] init];
    ...
    if (_title_string.length == 0 || [_title_string isEqual:[NSNull null]] || [_title_string isEqualToString:@"(null)"])
    {
        _title_string = @"";
    }
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"guanbi" object:nil];
    ...
    // Controls
    [nav.layer removeAllAnimations]; // Stop all animations on nav bar
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    //    [[UIApplication sharedApplication].keyWindow addSubview:nav];
    _viewIsActive = YES;
}
...
- (void)setNavBarAppearance:(BOOL)animated
{
    ...
    if ([[UINavigationBar class] respondsToSelector:@selector(appearance)]) {
        ...
        [nav setBackgroundImage:nil forBarMetrics:UIBarMetricsLandscapePhone];
    }
}
...
- (void)storePreviousNavBarAppearance
{
    ...
    self.previousNavBarTintColor = nav.tintColor;
    _previousNavBarStyle = nav.barStyle;
    if ([[UINavigationBar class] respondsToSelector:@selector(appearance)]) {
        self.navigationBarBackgroundImageDefault = [nav backgroundImageForBarMetrics:UIBarMetricsDefault];
        self.navigationBarBackgroundImageLandscapePhone = [nav backgroundImageForBarMetrics:UIBarMetricsLandscapePhone];
    }
}
...
- (void)restorePreviousNavBarAppearance:(BOOL)animated
{
    if (_didSavePreviousStateOfNavBar) {
        nav.tintColor = _previousNavBarTintColor;
        nav.barStyle = _previousNavBarStyle;
        if ([[UINavigationBar class] respondsToSelector:@selector(appearance)]) {
            [nav setBackgroundImage:_navigationBarBackgroundImageDefault forBarMetrics:UIBarMetricsDefault];
            [nav setBackgroundImage:_navigationBarBackgroundImageLandscapePhone forBarMetrics:UIBarMetricsLandscapePhone];
        }
...
- (void)updateNavigation
{
    ...
    {
        //		NavTitle.title = [NSString stringWithFormat:@"%i %@ %i", _currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), [self numberOfPhotos]];
        //
        //
        title_label.text = [NSString stringWithFormat:@"%d/%d  %@",_currentPageIndex+1,[self numberOfPhotos],self.title_string];
	} else
...
- (void)savePhoto
{
    ...
    if ([photo underlyingImage])
    {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"正在保存", @"Displayed with ellipsis as '正在保存...' when an item is in the process of being saved")]];
